...
def load_to_silver(df, silver_bucket):
    buffer = io.BytesIO()                                                                                                                                                     
    df.to_parquet(buffer)
    buffer.seek(0)                                                                                                                                                            
    client = storage.Client()                                                                                                                                                 
    bucket_obj = client.bucket(silver_bucket)
    blob_name = f'interest_rates/{date.today()}.parquet' 
...
def main():
    ...
    blob_loc = f'interest_rates/{date.today()}.json'
    ...
    load_to_silver(df, silver_bucket)
